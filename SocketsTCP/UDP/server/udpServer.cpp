   ...
   int send_bytes;
    ...
    while(recv_bytes>0){
       ...
       char _buffer[SIZE];
        ...
        std::cout<<"Message from client: "<<std::string(_buffer,0,recv_bytes+1)
                 <<std::endl;

        std::cout<<"Server get connect from addr :"<<inet_ntoa(clien_addr.sin_addr)
...
int udpServer::SendTo(int _fd,const char *_buffer, size_t buff_len, int _options) const
{
    return sendto(_fd,_buffer,buff_len,_options,(const sockaddr*)&serv_addr,addr_size);
}

int udpServer::RecvFrom(int _fd,char *_buffer, size_t buff_len,const sockaddr_in* _form,int _options)const
{

  socklen_t size_form=sizeof(*_form);

  return recvfrom(_fd,_buffer,buff_len,_options,(sockaddr*)&_form,&size_form);
